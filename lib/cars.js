...
const {esc,slug,name,tidy,n2,n1,n0}=V;
...
<a class="home" href="${site}/vs/">Comparisons</a></p>
<h1>${esc(nm)}</h1>
...
function order(all,vsMade){
  ...
  for(const k of order2) groups.get(k).sort((x,y)=>x.f.s60-y.f.s60||x.c.id.localeCompare(y.c.id));
...
function slugs(rows){
  const taken=new Map();
  for(const r of rows){
    let base=slug(name(r.c)), s=base, n=2;
...
function build(all,outDir,site,vsMade,limit){
  const ranked=slugs(order(all,vsMade));
  const pub = limit==null ? ranked : ranked.slice(0,limit);
  ...
  urls.unshift("/cars/");
  return {urls,carUrl,published:pub};
}
